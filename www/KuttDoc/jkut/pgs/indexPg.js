...
export  async  function mk(wg, pack)  {sys.$params(arguments.length, 2);
 ...
 function add( Trs,  Trees, ppath, space, linkPrefix)  {sys.$params(arguments.length, 5);
  ...
  arr.sort(Trees,function( t1,  t2)  {sys.$params(arguments.length, 2);
     return sys.$eq(t1[indexTree.type] , indexTree.kfile)
      ? sys.$eq(t2[indexTree.type] , indexTree.kfile)
  ...
  for (const  t  of sys.$forObject( Trees)) {
    if (sys.$eq(t[indexTree.type] , indexTree.kfile)) {
      arr.push(Trs,Q("tr")
        .add(Q("td")
      ...
      arr.push(Trs,Q("tr")
        .add(Q("td")
          .style(
              ...
              (sys.$eq(t[indexTree.type] , indexTree.library) ? "color:#600060;" : "")
            )
