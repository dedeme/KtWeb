...
export  async  function mk(wg, reload)  {sys.$params(arguments.length, 2);
  ...
  const songV =sys.$checkNull( [ssong]);
  ...
  audio.src =sys.$checkExists(audio.src,sys.$checkNull( "songs/" + songGroup + "/" + ssong[song.id]));
  ...
  timer.delay (1000, function()  {sys.$params(arguments.length, 0);
    ...
    timer.run(tm1, function()  {sys.$params(arguments.length, 0);
      ...
      if (sys.$eq(d , audio.currentTime) || sys.$eq(audio.currentTime , 0)) {
        ...
        const t =sys.$checkNull( (audio.duration * ssong[song.lapse]) / 100.0);
  ...
  timer.run (songTm, async  function()  {sys.$params(arguments.length, 0);
    if (audio.ended) {
      ...
      global.dbKeyV[0] =sys.$checkExists(global.dbKeyV[0],sys.$checkNull( dbKey));
      songGroupV[0] =sys.$checkExists(songGroupV[0],sys.$checkNull( group));
      songV[0] =sys.$checkExists(songV[0],sys.$checkNull( ssong));
      audio.src =sys.$checkExists(audio.src,sys.$checkNull( "songs/" + songGroup + "/" + ssong[song.id]));
