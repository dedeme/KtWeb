...
export  async  function mk(wg, reload)  {sys.$params(arguments.length, 2);
  const {dbKey,  picture, radioName, radioUrl} 
  = await  client.send({
    prg: cts.appName,
    source: "PinupsPg",
    rq: "idata"
  ...
  const img =sys.$checkNull( Visuals.img
    .att("src", "img/pinups/" + picture[pict.id]))
  ;
  ...
  const InfoPicture =sys.$checkNull( infoWg.mk( -485, infoWg.pictureWg("-", picture)));
  ...
  const div =sys.$checkNull( Visuals.div
    .setStyle(
      "background-image",
      "url('img/pinups/" + picture[pict.id] + "')"
    )
    .add(InfoPicture.wg)
    .add(InfoSong.wg)
    .add(PictTime.wg)
  ...
  timer.run (pictTm, async  function()  {sys.$params(arguments.length, 0);
    ...
    global.dbKeyV[0] = dbKey;
     const p =sys.$checkNull( pictV[0]);
    if (sys.$neq(picture[pict.id] , p[pict.id])) {
      pictV[0] = picture;
