...
export  function mk(wg, selectedYear, isUntil, selectedMonth,
   ...
   function initialBalance()  {sys.$params(arguments.length, 0);  return Q("table")
      ...
      .add(Q("tr")
        .add(Q("td")
          ...
          .text(math.toIso(sys.asBool(isUntil) ? balance : 0, 2))))
    ;};

  
   function finalBalance()  {sys.$params(arguments.length, 0);
    const real =sys.$checkNull( diary.totalAmount(
      Diary,sys.asBool( isUntil) ? 0 : selectedMonth - 1, selectedMonth
    ));
    const budgeted =sys.$checkNull( budget.totalAmount(
      Budget, Plan,sys.asBool( isUntil) ? 0 : selectedMonth - 1, selectedMonth
    ));
      ...
      .add(Q("table")
        ...
        .add(Q("tr")
          ...
          .add(Q("td")
            .klass("frameNm")
            .style("background-color:#f9f9f9")
            .text(math.toIso((sys.asBool(isUntil) ? balance : 0) + budgeted, 2)))
          .add(Q("td")
            .klass("frameNm")
            .style("background-color:#f9f9f9")
            .text(math.toIso((sys.asBool(isUntil) ? balance : 0) + real, 2)))
          .add(Q("td")
            .klass("frameNm")
            .style("background-color:#f9f9f9")
  ...
  showOp[0] =sys.$checkExists(showOp[0], function()  {sys.$params(arguments.length, 0);
    const Lopts =sys.$checkNull( [sys.asBool(
      isUntil)
    ...
    const Ropts =sys.$checkNull( [sys.asBool(
      isEditionV[0])
        ? menu.toption("", II("View"), changeEdition)
        : menu.toption("", II("Edit"), changeEdition)
    ]);
    const menuWg =sys.$checkNull( menu.mk(Lopts, Ropts, monthName, false));

    finalBalance();

    if (sys.asBool(isEditionV[0])) {
      budgetEdit.mk(
