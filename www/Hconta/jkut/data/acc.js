...
export  function toJs()  {sys.$params(arguments.length, 0);  return [
    ...
    dic.fromArr(arr.map(
      dic.toArr(accounts()),
      function(Kv)  {sys.$params(arguments.length, 1);  return [Kv[0], accValue.toJs(Kv[1])];}
    )),
    subaccounts(),
...
export  function fromJs(A)  {sys.$params(arguments.length, 1);
  if (sys.asBool(!sys.asBool(A))) return; 

  SubgroupsV[0] =sys.$checkExists(SubgroupsV[0],sys.$checkNull( A[0]));
  AccountsV[0] =sys.$checkExists(AccountsV[0],sys.$checkNull( dic.fromArr(arr.map(
      dic.toArr(A[1]),
      function(Kv)  {sys.$params(arguments.length, 1);  return [Kv[0], accValue.fromJs(Kv[1])];}
    ))));
...
export  function accFormat(ac)  {sys.$params(arguments.length, 1); return sys.asBool( str.len(ac) > 3) ? sys.$slice(ac,null,3) + "." + sys.$slice(ac,3,null) : ac;};
...
export  function subgroupAdd(id, description)  {sys.$params(arguments.length, 2);
  if (sys.asBool(dic.hasKey(SubgroupsV[0], id)))
...
export  function subgroupMod(oldId, newId, description)  {sys.$params(arguments.length, 3);
  ...
  if (sys.asBool(sys.$eq(oldId , newId))) {
    dic.put(Subs, newId, description);
     return "";
  }
  if (sys.asBool(dic.hasKey(Subs, newId)))
...
export  function accountAdd(id, description, summary)  {sys.$params(arguments.length, 3);
  if (sys.asBool(dic.hasKey(AccountsV[0], id)))
...
export  function accountMod(oldId, newId, description, summary)  {sys.$params(arguments.length, 4);
  ...
  if (sys.asBool(dic.hasKey(As, newId)))
...
export  function subaccountAdd(id, description)  {sys.$params(arguments.length, 2);
  if (sys.asBool(dic.hasKey(SubaccountsV[0], id)))
...
export  function subaccountMod(oldId, newId, description)  {sys.$params(arguments.length, 3);
  ...
  for (let i  of sys.$forObject( usedSubaccounts(oldId))) {
    ...
    if (sys.asBool(dic.hasKey(E.credits, oldId))) {
      const v =sys.$checkNull( E.credits[oldId]);
...
export  function usedAccs(ac)  {sys.$params(arguments.length, 1);
  ...
  arr.eachIx(DiaryV[0], function(E, i)  {sys.$params(arguments.length, 2);
    if (sys.asBool(
      sys.asBool(arr.any(dic.toArr(E.debits), function(Kv)  {sys.$params(arguments.length, 1);  return sys.$eq(sys.$slice(Kv[0],null,len) , ac);})) ||
      sys.asBool(arr.any(dic.toArr(E.credits), function(Kv)  {sys.$params(arguments.length, 1);  return sys.$eq(sys.$slice(Kv[0],null,len) , ac);})))
    )
      arr.push(R, i);
  });
   return R;
...
export  function usedSubaccounts(subacc)  {sys.$params(arguments.length, 1);
  const R =sys.$checkNull( []); 
  arr.eachIx(DiaryV[0], function(E, i)  {sys.$params(arguments.length, 2);
    if (sys.asBool(sys.asBool(dic.hasKey(E.debits, subacc)) || sys.asBool(dic.hasKey(E.credits, subacc))))
      arr.push(R, i);}
  );
   return R;
};
...
export  function descriptionOf(ac)  {sys.$params(arguments.length, 1);
  ...
  if (sys.asBool(sys.$eq(lg , 3))) {
    return sys.asBool( dic.hasKey(AccountsV[0], ac)) ? AccountsV[0][ac].description : "";
  } else {
    const D =sys.$checkNull(sys.asBool( sys.$eq(lg , 1)) ? groups()
      :sys.asBool( sys.$eq(lg , 2)) ? SubgroupsV[0]
      : SubaccountsV[0])
    ;
    return sys.asBool( dic.hasKey(D, ac)) ? D[ac]: "";
  }
};
...
export  function available(ac, extra)  {sys.$params(arguments.length, 2);
  ...
  const it =sys.$checkNull(sys.asBool( sys.$eq(str.len(ac) , 3))
    ? iter.map(iter.$range(0,26), function(i)  {sys.$params(arguments.length, 1); return sys.asBool( i < 10) ? "0" + i : "" + i;})
  ...
  if (sys.asBool(sys.$neq(extra , ""))) arr.push(R, extra);
...
export  function addDiary(Entry)  {sys.$params(arguments.length, 1);
  const date =sys.$checkNull( Entry.date);
  const R =sys.$checkNull( []); 
...
export  function close(newYear)  {sys.$params(arguments.length, 1);
  const Accs =sys.$checkNull( {}); 
   ...
   function add(ac , am)  {sys.$params(arguments.length, 2);
    if (sys.asBool(dic.hasKey(Accs, ac))) Accs[ac] +=sys.$checkExists(Accs[ac],sys.$checkNull( am));
  ...
  for (let E  of sys.$forObject( DiaryV[0])) {
    arr.each(
      arr.filter(
        dic.toArr(E.debits),
        function(Kv)  {sys.$params(arguments.length, 1);  return sys.$eq(Accounts[sys.$slice(Kv[0],0,3)].summary[0] , "B");}
      ),
