  ...
  arr.sort(DateGroups, function(d1, d2)  {sys.$params(arguments.length, 2);  return d1 > d2;});
   ...
   function twoChart(PsEvs)  {sys.$params(arguments.length, 1);
    ...
    const color =sys.$checkNull( fns.valueColor(max, min));
     ...
     return Q("table")
      .klass("flat")
      .adds(iter.map(iter.$range(0,rows), function(row)  {sys.$params(arguments.length, 1);  return Q("tr")
          .adds(iter.map(iter.$range(0,cols), function(col)   {sys.$params(arguments.length, 1);
