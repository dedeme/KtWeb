...
export  async  function mk(wg, type0)  {sys.$params(arguments.length, 2);
  const type =sys.$checkNull( sys.$neq(type0 , "assets") && sys.$neq(type0 , "profits") ? "points" : type0);
  ...
  arr.sort(DateGroups, function(d1, d2)  {sys.$params(arguments.length, 2);  return d1 > d2;});
   ...
   function rowGroups(start, end)  {sys.$params(arguments.length, 2);
    ...
    for (let i = start;i < end; ++i) groupTable(Tds[i - start], DateGroups[i]);
     return Q("table")
      .att("align", "center")
      .add(Q("tr").adds(Tds))
    ;
