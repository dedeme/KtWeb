...
const II =sys.$checkNull( i18n.tlt);
 ...
 async  function mk(wg)  {sys.$params(arguments.length, 1);
  ...
  const sel1 =sys.$checkNull(sys.asBool( sys.asBool(daysWinOp) && sys.asBool(sys.$eq(sys.$slice(daysWinOp[0],1,null) , "ranking"))) ? daysWinOp[0] : daysWin);
   ...
   function center()  {sys.$params(arguments.length, 0);
    if (sys.asBool(!sys.asBool(ui.confirm(II("Change near start corner for every group?"))))) return;
  ...
  for (const d  of sys.$forObject( cts.DaysWin)) {
  ...
  for (const d  of sys.$forObject( cts.DaysLoss)) {
    arr.push(Lopts2, menu.tlink(sel1 + "&" + d + "&" + sel3, d, []));
    arr.push(Lopts2, menu.separator());
  }
  arr.pop(Lopts2);

  const Ropts2 =sys.$checkNull( [menu.ioption("---", "center", center)]);

  const menuWg2 =sys.$checkNull( menu.mk(Lopts2, Ropts2, daysWin + "&" + daysLoss + "&" + sel3, false));
