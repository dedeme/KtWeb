...
export  async  function mk(wg)  {sys.$params(arguments.length, 1);
  ...
  if (!sys.asBool(AllEs)) {
    const msg =sys.$checkNull( II("Profits values are missing"));
    ui.alert(msg);
    throw new Error( msg);
  }
  ...
  for (const  e  of sys.$forObject( AllEs)) {
    ...
    if (sys.$neq(sys.$slice(e[profitsEntry.date],null,4) , sys.$slice(e1[profitsEntry.date],null,4))) {
      correctsSumV[0] +=sys.$checkNull( e1[profitsEntry.acc]);
  ...
  const ixV = [ -1];
  for ( const [i, e]  of sys.$forObject2( AllEs)) if (sys.$slice(e[profitsEntry.date],null,6) >=  month) {
    ixV[0] = i;
    break;
  }
   const E00s =sys.$checkNull( ixV[0] >= 0 ? sys.$slice(AllEs,ixV[0],null) : []);
   const E0s =sys.$checkNull( arr.size(E00s) > 0 ? E00s : sys.$slice(AllEs, -1,null));

  if (ixV[0] <= 0){
    arr.unshift(E0s,AllEs[0]);
  } else {
     const e0 =sys.$checkNull( AllEs[ixV[0] - 1]);
     const e1 =sys.$checkNull( E0s[0]);

    if (sys.$eq(sys.$slice(e0[profitsEntry.date],null,4) , sys.$slice(e1[profitsEntry.date],null,4))) {
      arr.unshift(E0s,e0);
    } else {
       const newE =sys.$checkNull( profitsEntry.mk(
        e0[profitsEntry.date], e0[profitsEntry.total] - e0[profitsEntry.acc], 0, e0[profitsEntry.risk] - e0[profitsEntry.acc]
      ));
      arr.unshift(E0s,newE);
    }
  }
  ...
  ixV[0] =  -1;
  for ( const [i, e]  of sys.$forObject2( AllEs)) if (sys.$slice(e[profitsEntry.date],null,4) >=  year) {
    ixV[0] = i;
    break;
  }
   const E10s =sys.$checkNull( ixV[0] >= 0 ? sys.$slice(AllEs,ixV[0],null) : []);
   const E1s =sys.$checkNull( arr.size(E10s) > 0 ? E10s : sys.$slice(AllEs, -1,null));

  if (ixV[0] <= 0){
    arr.unshift(E1s,AllEs[0]);
  } else {
     const e0 =sys.$checkNull( AllEs[ixV[0] - 1]);
      const newE =sys.$checkNull( profitsEntry.mk(
       e0[profitsEntry.date], e0[profitsEntry.total] - e0[profitsEntry.acc], 0, e0[profitsEntry.risk] - e0[profitsEntry.acc]
     ));
     arr.unshift(E1s,newE);
  }
  ...
  Data3.mapLabel = function(lb, i)  {sys.$params(arguments.length, 2);  return sys.$slice(lb, -2,null);};
