...
export  function mk2(wg, AllEs)  {sys.$params(arguments.length, 2);
  ...
  for (const E  of sys.$forObject( AllEs)) {
    ...
    if (sys.$neq(sys.$slice(E.date,null,4) , sys.$slice(E1.date,null,4))) {
  ...
  const now =sys.$checkNull( time.fromStr(e.date)[0]);
  ...
  const E00s =sys.$checkNull( arr.dropWhile(AllEs, function(E)  {sys.$params(arguments.length, 1);  return sys.$slice(E.date,null,6) <  month;}));
  ...
  const Data0 =sys.$checkNull( oldChart.mkData(Lb0s, Val0s, Att0s));
  ...
  const E10s =sys.$checkNull( arr.dropWhile(AllEs, function(E)  {sys.$params(arguments.length, 1);  return sys.$slice(E.date,null,4) <  year;}));
  ...
  const Data1 =sys.$checkNull( oldChart.mkData(Lb1s, Val1s, Att1s));
  ...
  const Data2 =sys.$checkNull( oldChart.mkData(Lb2s, Val2s, Att2s));
  ...
  if (sys.$neq(arr.peek(E30s).date , arr.peek(CEs).date))
  ...
  const Data3 =sys.$checkNull( oldChart.mkData(Lb3s, Val3s, Att3s));
