...
export  function mk(wg,  Cos,  Closes,  rs)  {sys.$params(arguments.length, 4);
  ...
  if (arr.size(Orders) > 0) {
    ...
    for (const  o  of sys.$forObject( Orders)) {
      ...
      switch (o[order.type]) {
        case global.orderSellV[0]:{ {
          ...
          cashV[0] +=sys.$checkExists(cashV[0],sys.$checkNull( broker.sell(o[order.stocks], o[order.price])));
        ...
        case global.orderBuyV[0]:{ {
          ...
          cashV[0] -=sys.$checkExists(cashV[0],sys.$checkNull( broker.buy(o[order.stocks], o[order.price])));
        }break;}
      }
    }
    arr.push(Trs,
    ...
    assetsV[0] =sys.$checkExists(assetsV[0],sys.$checkNull( arr.reduce(
      dic.toArr(Portfolio),
      cashV[0],
