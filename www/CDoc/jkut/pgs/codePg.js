...
const reserved = "asm auto break bool case char const continue " +
...
export  async  function mk(wg, module, path, anchor)   {sys.$params(arguments.length, 4);
  const isHeader = !sys.asBool(str.starts(anchor, "hp::"));
  ...
  const stateV = [stCode];
   ...
   function processCode(l)  {sys.$params(arguments.length, 1);
    ...
    for (const w  of sys.$forObject( str.split(reserved, " "))) {
      ...
      while (sys.$neq(ixV[0] ,  -1)) {
        ...
        const ix2 = ixV[0] + str.len(w);
        if ((sys.$eq(ixV[0] , 0) || isNotId(r[ixV[0] - 1])) &&
            (sys.$eq(ixV[0] , str.len(r)) || isNotId(r[ix2]))
        ) {
          rV[0] =sys.$checkExists(rV[0], sys.$slice(r,null,ixV[0]) + "<span class='reserved'>" + w +
    ...
    for (const w  of sys.$forObject( str.split(directive, " "))) {
      ...
      while (sys.$neq(ixV[0] ,  -1)) {
        const ix2 = ixV[0] + str.len(w);
        rV[0] =sys.$checkExists(rV[0], sys.$slice(rV[0],null,ixV[0]) + "<span class='annotation'>" + w +
   ...
   function processLine(l)  {sys.$params(arguments.length, 1);
    if (sys.$eq(stateV[0] , stLong)) { 
      ...
      if (sys.$eq(qix ,  -1)) {
        rightV[0] +=sys.$checkExists(rightV[0], toHtml(l) + charQuotesV[0] + "</span>");
        newLine();
        stateV[0] =sys.$checkExists(stateV[0], stCode);
        return;
      }
      ...
      if (sys.$eq(rV[0] , 1)) { 
        ...
        const l2= sys.$slice(l,posV[0] + 2,null);
        if (str.starts(l2, "*")) {
        ...
        rightV[0] +=sys.$checkExists(rightV[0], "<span class='quote2'>\"");
        processLine(sys.$slice(l,posV[0] + 1,null));
      } else if (sys.$eq(rV[0] , 4)) { 
        processCode(sys.$slice(l,null,posV[0]));
        stateV[0] =sys.$checkExists(stateV[0], stQ);
        charQuotesV[0] =sys.$checkExists(charQuotesV[0], "'");
        rightV[0] +=sys.$checkExists(rightV[0], "<span class='quote1'>'");
        processLine(sys.$slice(l,posV[0] + 1,null));
      } else {
        processCode(l);
        newLine();
  ...
  const eOp = sys.$null((window.document.getElementById(anchor)));
